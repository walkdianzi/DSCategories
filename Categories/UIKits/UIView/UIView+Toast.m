...
static const NSString * CSToastAnimationImageKey= @"CSToastAnimationImageKey";
...
-(void)makeToastAnimationImage{
    ...
    [self makeToastAnimationImageWithSize:CGSizeMake(45, 45) isdismiss:YES];
}

- (void)makeToastAnimationImageWithSize:(CGSize)size isdismiss:(BOOL)isdismiss{
    
    [self makeToastAnimationImageWithSize:size withImageArray:nil isdismiss:YES];
}
...
- (void)makeToastAnimationImageWithSize:(CGSize)size withImageArray:(NSArray *)imageArray isdismiss:(BOOL)isdismiss{
    ...
    if (imageArray) {
        for (int i=0; i<[imageArray count]; i++) {
            if ([imageArray objectAtIndex:i] && [[imageArray objectAtIndex:i] isKindOfClass:[NSString class]]) {
                UIImage *image = [UIImage imageNamed:[imageArray objectAtIndex:i]];
                [animationImageArray addObject:image];
            }else if ([imageArray objectAtIndex:i] && [[imageArray objectAtIndex:i] isKindOfClass:[UIImage class]]){
        ...
        while (image) {
            [animationImageArray addObject:image];
            i+=1;
            image = [self imageNamed:[NSString stringWithFormat:@"toast_%ld",(long)i] ofBundle:@"DSCategories.bundle"];
        }
    }
    
    imageView.animationImages= animationImageArray;
    //设置动画总时间
    ...
    // associate the activity view with self
    objc_setAssociatedObject (self, &CSToastAnimationImageKey, activityView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
